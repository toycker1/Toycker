-- Fix: gift wrap fee missing from order total when gift_wrap_line cart item fails to persist.
-- Strategy: if no gift_wrap_line row exists in cart_items at order creation time, fall back to
-- the gift_wrap metadata on the main product item to derive and add the fee.
-- Also stores gift_wrap_amount explicitly in order metadata so the admin panel can display it.
