-- Fix: keep removed gift wrap from being counted during order creation.
-- Strategy: use actual gift_wrap_line cart items as the source of truth for gift wrap.
-- Product-line gift_wrap metadata is only a display marker and must not resurrect removed gift wrap.
...
BEGIN
  ...
  -- Gift wrap is counted only from actual gift_wrap_line cart items. Product-line
  -- gift_wrap metadata is only a UI marker and may remain after the add-on is removed.
  SELECT COALESCE(
    SUM(
